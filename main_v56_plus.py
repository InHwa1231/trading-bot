"""V56-plus 백테스트 실행 - V56 기반 수익률 강화"""
import sys, os, time
...
result_path = os.path.join(result_dir, f"V56plus_{timestamp}")
os.makedirs(result_path, exist_ok=True)
sys.stdout = TeeOutput(os.path.join(result_path, "backtest_log.txt"))

from backtest.engine_v56_plus import run_portfolio_backtest
import pandas as pd
...
print("\n" + "=" * 70)
print(f"  V56-plus 백테스트 결과")
print(f"  (V56 기반 + LINK추가 + 레버리지4-6x + 3포지션 + 쿨다운12h)")
print("=" * 70)
...
# V56 대비 비교
print(f"\n  [V56 대비 비교]")
print(f"    V56:      $357K | 월 3.60% | PF 2.28 | MDD -32.6%")
print(f"    V56-plus: ${final/1000:.0f}K | 월 {monthly_avg:.2f}% | PF {pf:.2f} | MDD {stats['max_drawdown']:.1f}%")
...
try:
    ...
    fig.suptitle('V56-plus: V56 Base + LINK + Lev 4-6x + 3 Positions', fontsize=14, fontweight='bold')

    # 1. 자산 곡선
    axes[0, 0].plot(history, linewidth=0.8, color='blue', label='V56-plus')
