"""
V56-plus 백테스트 엔진 - V56 검증 로직 기반 수익률 강화

V56 대비 변경사항:
1. 레버리지: 3-4x → 4-6x (ADX 기반)
2. 자산 추가: LINK (V57 테스트에서 PF 4.09)
3. 최대 포지션: 2 → 3
4. 쿨다운: 24h → 12h (6봉 → 3봉)
5. 분할 익절: 마진 8% → 6%
나머지 로직은 V56과 100% 동일
...
# ── 글로벌 파라미터 ──
ATR_TRAIL_MULT_EMA = 4.0       # V56 검증값 유지
ATR_TRAIL_MULT_DON = 3.0       # V56 검증값 유지
COOLDOWN = 3                    # 12시간 쿨다운 (V56-plus 검증값)
MAX_HOLD = 120
PARTIAL_TRIGGER = 6.0           # 마진 6% 수익 시 분할 (V56-plus 검증값)
PARTIAL_RATIO = 0.4             # 40% 분할 (V56-plus 검증값)
...
def run_portfolio_backtest(initial_budget=100_000):
    ...
    print(f"\n[V56-plus] 4h | EMA+돈치안 | {len(asset_data)}자산 | 레버리지4-6x | {MAX_POSITIONS}포지션")
    ...
    for ts in timeline:
        ...
        for pos in positions[:]:
            ...
            if not pos.partial_done and margin_ret >= PARTIAL_TRIGGER:
                ...
                print(f"{time_str} | {pos.asset:<4} | {pos.strategy:<3} | PART   | {cur_close:>9,.0f} | {pr:>6.1f}% | {fee:>5.0f} | {cash:>11,.0f} | 40%@{PARTIAL_TRIGGER}%")
            ...
            if hit_stop or (is_exit_signal and candles_held >= 3) or candles_held >= MAX_HOLD:
                ...
                print(f"{time_str} | {pos.asset:<4} | {pos.strategy:<3} | EXIT   | {fp:>9,.0f} | {final_ret:>6.1f}% | {fee:>5.0f} | {cash:>11,.0f} | {remark} ({candles_held}bars)")
