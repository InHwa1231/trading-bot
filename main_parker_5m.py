...
result_path = os.path.join(result_dir, f"Parker5m_{timestamp}")
os.makedirs(result_path, exist_ok=True)
sys.stdout = TeeOutput(os.path.join(result_path, "backtest_log.txt"))

from backtest.engine_parker_5m import run_portfolio_backtest
import pandas as pd
...
print("\n" + "=" * 70)
print(f"  Parker Brooks 5분봉 스캘핑 백테스트 결과")
print(f"  (VWAP + EMA + Volume Profile | BTC/ETH/SOL | 5m)")
print("=" * 70)
...
print(f"  거래일: {total_days}일 | 수익일: {positive_days}일 ({positive_days/max(total_days,1)*100:.1f}%)")
print(f"  실행 시간: {elapsed:.1f}초")

# 비교
print(f"\n  [V56-plus(4h) 대비 비교]")
print(f"    V56-plus 4h: $389K | 월 8.01% | PF 2.28 | MDD -39.2% | 122건")
print(f"    Parker   5m: ${final/1000:.0f}K | 월 {monthly_avg:.2f}% | PF {pf:.2f} | MDD {stats['max_drawdown']:.1f}% | {stats['total']}건")
...
try:
    ...
    fig.suptitle('Parker Brooks 5m Scalping: VWAP+EMA+VP (BTC/ETH/SOL)', fontsize=14, fontweight='bold')

    axes[0, 0].plot(history, linewidth=0.8, color='blue')
    ...
    axes[1, 1].set_title('Trades by Asset/Side')
    axes[1, 1].legend()
