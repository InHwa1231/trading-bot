...
def calc_daily_regime_shifted(daily_df):
    """
    [V54 개선] 더 엄격한 레짐 감지
    - ADX 임계값 20 → 25 (더 강한 추세 요구)
    - EMA20 기울기 방향 추가 확인 (EMA20이 실제로 움직이는 방향)
    - 이로써 '상승 추세처럼 보이지만 실제론 횡보'인 구간을 RANGE로 분류
    """
    ddf = daily_df.copy()
    c, h, l = ddf['Close'], ddf['High'], ddf['Low']

    e20 = EMAIndicator(c, 20).ema_indicator()
    e50 = EMAIndicator(c, 50).ema_indicator()
    # EMA20 기울기: 5일 전 대비 방향 확인
    e20_slope = e20 - e20.shift(5)

    adx_obj = ADXIndicator(h, l, c, 14)
    adx     = adx_obj.adx()

    def get_regime(r):
        if pd.isna(r.adx) or r.adx < 25:
            return "RANGE"
        # BULL: EMA20 > EMA50 이고 EMA20이 상승 중
        if r.e20 > r.e50 and r.e20_slope > 0:
            return "BULL"
        # BEAR: EMA20 < EMA50 이고 EMA20이 하락 중
        if r.e20 < r.e50 and r.e20_slope < 0:
            return "BEAR"
        return "RANGE"
...
def prep_data_vectorized(data_btc, daily_df, data_15m, data_1h):
    ...
    # ── [V54 추가] 15분봉 추세 확인 (중간 타임프레임 필터) ──
    if not data_15m.empty:
        d15m        = data_15m.copy()
        d15m['e20'] = EMAIndicator(d15m['Close'], 20).ema_indicator()
        d15m['e50'] = EMAIndicator(d15m['Close'], 50).ema_indicator()
        d15m['m15_bull'] = (d15m['e20'] > d15m['e50']).shift(1, fill_value=False)
        d15m['m15_bear'] = (d15m['e20'] < d15m['e50']).shift(1, fill_value=False)
        m15 = d15m[['m15_bull', 'm15_bear']].reindex(df.index, method='ffill')
        df['m15_bull'] = m15['m15_bull'].fillna(False).astype(bool)
        df['m15_bear'] = m15['m15_bear'].fillna(False).astype(bool)
    else:
        df['m15_bull'] = True   # 15m 데이터 없으면 필터링 안 함
        df['m15_bear'] = True
    ...
    # ── 원시 진입/청산 조건 ──
    # [V54] BULL: 1h + 15m 이중 추세 필터 추가
    df['raw_buy_bull'] = (
        df['h1_bull'] &
        df['m15_bull'] &                            # [V54] 15분봉 상승 추세 확인
    ...
    # [V54] BEAR: 1h + 15m 이중 추세 필터 추가
    df['raw_short_bear'] = (
        df['h1_bear'] &
        df['m15_bear'] &                            # [V54] 15분봉 하락 추세 확인
        (df['ema9'] < df['ema21']) & (df['ema21'] < df['ema50']) &
        (h >= df['ema21'] * 0.998) & (c < df['ema9']) &
        df['obv_falling'] &
        (df['rsi'] < 55) & (df['rsi'] > 35) &
        df['atr_ok'] &
        df['below_recent_high']
    )
