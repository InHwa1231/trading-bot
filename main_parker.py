"""Parker Brooks 스타일 백테스트 실행 - VWAP+EMA+Volume Profile"""
import sys, os, time
...
result_path = os.path.join(result_dir, f"Parker_{timestamp}")
os.makedirs(result_path, exist_ok=True)
sys.stdout = TeeOutput(os.path.join(result_path, "backtest_log.txt"))

from backtest.engine_parker import run_portfolio_backtest
import pandas as pd
...
print("\n" + "=" * 70)
print(f"  Parker Brooks 스타일 백테스트 결과")
print(f"  (VWAP + EMA + Volume Profile | BTC/ETH/SOL | 롱+숏)")
print("=" * 70)
...
print(f"    Parker:   ${final/1000:.0f}K | 월 {monthly_avg:.2f}% | PF {pf:.2f} | MDD {stats['max_drawdown']:.1f}%")
...
try:
    ...
    fig.suptitle('Parker Brooks: VWAP + EMA + Volume Profile (BTC/ETH/SOL)', fontsize=14, fontweight='bold')
